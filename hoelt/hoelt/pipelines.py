...
import pymysql
from hoelt.settings import tableName

class HoeltPipeline(object):
    ...
    def process_item(self, item, spider):
        try:
            sql = """insert into {0}(avatar,nickname,`name`,datails,joindata) VALUES ("{img1}","{city}","{name}","{address}","{phone}")""".format(self.tableName,**item)
            self.cursor.execute(sql)
            self.db.commit()
        except:
            print("数据已经重复，不存入")
